...
struct Pseudodesc gdt_ptr, idt_ptr;//伪gdtr、idtr
...
init_gdt()
{
	...
	// 数据段(ds,es,fs,ss)
	init_segment(&gdt[2], 0, 0xfffff, DA_DRW | DA_32 | DA_LIMIT_4K);//没加DA_DPL的限制，所以为0,是最高特权级
	// 显存段(gs)
	...
	gdt_ptr.pd_base = (u32)gdt;		//将伪GDTR的信息填好
...
{
	// 异常处理
	//选择符已经统一置为SELECTOR_FLAT_C，所以不作为参数了
	...
	idt_ptr.pd_base = (u32)idt;		//将伪IDTR的信息填好
