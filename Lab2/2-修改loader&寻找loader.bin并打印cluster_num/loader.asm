    org  0400h                       ; Boot 状态, Bios 将把 Boot Sector 加载到 0:7C00h 处并开始执行
...
;字符串
;----------------------------------------------------------------------------
LoaderFileName            db    "aA1     txt", 0  ; LOADER.BIN 的文件名(为什么中间有空格请RTFM)
; 为简化代码, 下面每个字符串的长度均为 MessageLength
MessageLength             equ    9
BootMessage:              db    "Loadering"    ; 9字节, 不够则用空格补齐. 序号 0
Message1                  db    "Find it! "    ; 9字节, 不够则用空格补齐. 序号 1
Message2                  db    "Read Fail"    ; 9字节, 不够则用空格补齐. 序号 2
Message3                  db    "No Loader"    ; 9字节, 不够则用空格补齐. 序号 3
clustername:              db "CLUSTER NUMBER:",0
top_10_char_of_AA1_TXT: db "top_10_char_of_AA1_TXT:",0
;============================================================================
...
printoutstring:
	...
	mov ah,0x07 ;设置所有字符的显示模式：黑底白字
	shl di,1    ;将屏幕上字符串的开始位置转化成显存的开始字节
.loop_print_char:
	mov al,[ds:si] ;将ds:si指向的字符传送到al中

	cmp al,0 ;判断字符是否为0
	je .end ;如果是0，则跳转到.end

	mov [gs:di],ax ;不是0，则将al中的字符传送到显存中，实现si指向字符的打印
	inc si	 ;si指向下一个字符
	add di,2	;di指向下一个字符对应的显存
	jmp .loop_print_char
...
;将dx中的数字簇号(dl)转换成字符串,结果在num_str中
numbertostring:
	pusha
...
.print_aa1_txt_top_10_char:
	push ds
	pusha
	mov si,top_10_char_of_AA1_TXT
	mov di,560
	call print_str
	...
	mov di,640
	mov cx,10 ;cx表示共打印10个字符
	call print_cx_num_char
	popa
	pop ds    
