/*****************************************************
*			vfork.c			//add by visual 2016.5.25
*系统调用vfork()功能实现部分sys_vfork()
********************************************************/
#include "type.h"
#include "const.h"
#include "protect.h"
...
static int vfork_mem_cpy(u32 ppid,u32 pid);
static int vfork_pcb_cpy(PROCESS* p_child);
static int vfork_update_info(PROCESS* p_child);


/**********************************************************
*		sys_vfork			//add by visual 2016.5.25
*系统调用sys_vfork的具体实现部分
*************************************************************/
int sys_vfork()
{
	PROCESS* p_child;
	...
	if( 0==p_child )
	{
		vga_write_str_color("PCB NULL,vfork faild!",0x74);
		return -1;
	}
	else
	{
		...
		vfork_pcb_cpy(p_child);

		/**************复制线性内存，包括堆、栈、代码数据等等***********************/
		vfork_mem_cpy(p_proc_current->task.pid,p_child->task.pid);
		
		/**************更新进程树标识info信息************************/
		vfork_update_info(p_child);
	
		/************修改子进程的名字***************/		
		strcpy(p_child->task.p_name,"vfork");	// 所有的子进程都叫vfork
		...
		*((u32*)(p_reg + EAXREG - P_STACKTOP)) = p_child->task.regs.eax;	//added by xw, 17/12/11
		...
		vga_write_str_color("[vfork success:",0x72);
...
/**********************************************************
*		vfork_mem_cpy			//add by visual 2016.5.24
*复制父进程的一系列内存数据
*************************************************************/
static int vfork_mem_cpy(u32 ppid,u32 pid)
{
	...
	{	/*
		...
		u32 paddr = get_page_phy_addr(ppid,addr_lin);
		lin_mapping_phy(addr_lin,//线性地址
						paddr,//物理地址，为MAX_UNSIGNED_INT时，由该函数自动分配物理内存
						pid,//要挂载的进程的pid，子进程的pid
						PG_P  | PG_USU | PG_RWW,//页目录属性，一般都为可读写
						PG_P  | PG_USU | PG_RWR);//页表属性，代码是只读的	

		PROCESS *pro = &proc_table[ppid];
		PageList pgfault;
	...
	{	/*
		...
		PROCESS *pro = &proc_table[ppid];
		PageList pgfault;
		for (LinkList p = (&(pro->task.memmap.vpage_pages))->next; p != &(pro->task.memmap.vpage_pages); p = p->next)
		{
			PageList page = getPageEntry(p);
			if (paddr == page->p_addr && page->flag == PG_ALLOCATED)
			{
				page->flag = PG_COW;
				page->segment = MM_AREA_VPAGE;
				break;
			}
		}
	}
	...
	{	/*
		...
		PROCESS *pro = &proc_table[ppid];
		PageList pgfault;
		for (LinkList p = (&(pro->task.memmap.heap_pages))->next; p != &(pro->task.memmap.heap_pages); p = p->next)
		{
			PageList page = getPageEntry(p);
			if (paddr == page->p_addr && page->flag == PG_ALLOCATED)
			{
				page->flag = PG_COW;
				page->segment = MM_AREA_HEAP;
				break;
			}
		}
	}	
	...
	{	/*
		...
		PROCESS *pro = &proc_table[ppid];
		PageList pgfault;
		for (LinkList p = (&(pro->task.memmap.arg_pages))->next; p != &(pro->task.memmap.arg_pages); p = p->next)
		{
			PageList page = getPageEntry(p);
			if (paddr == page->p_addr && page->flag == PG_ALLOCATED)
			{
				page->flag = PG_COW;
				page->segment = MM_AREA_ARG;
				break;
			}
		}
	}
...
/**********************************************************
*		vfork_pcb_cpy			//add by visual 2016.5.26
...
/**********************************************************
*		vfork_update_info			//add by visual 2016.5.26
*更新父进程和子进程的进程树标识info
*************************************************************/
static int vfork_update_info(PROCESS* p_child)
{
