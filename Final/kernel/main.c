...
// add by pjt & ljc ---------------------------------------------------------------------------------
 		InitList(&p_proc->task.pages);
		InitList(&p_proc->task.memmap.text_pages);
		InitList(&p_proc->task.memmap.data_pages);
		InitList(&p_proc->task.memmap.vpage_pages);
		InitList(&p_proc->task.memmap.heap_pages);
		InitList(&p_proc->task.memmap.stack_pages);
		InitList(&p_proc->task.memmap.arg_pages);
...
// add by pjt & ljc
			PageList page = K_PHY2LIN((void *)do_kmalloc(sizeof(struct PageNode)));
			page->v_addr = AddrLin & 0xFFFFF000;
			page->flag = PG_UNALLOCATED;
			page->segment = MM_AREA_STACK;
			InsertNode(&p_proc->task.pages, &(page->page_node));
			InsertNode(&p_proc->task.memmap.stack_pages, &(page->memmap_node));
