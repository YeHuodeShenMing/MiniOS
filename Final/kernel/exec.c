...
int exec_lazyload(u32 fd, const Elf32_Ehdr *ehdr, const Elf32_Phdr phdr[]);
int map_elf_pglist(int fd, u32 offset, u32 filesz, u32 vaddr, u32 memsz, int area);
...
// add by pjt & ljc
	// 这里需要增加一个条件确保一开始加载shell时不使用懒加载，因为用了懒加载就会出现很奇怪的问题，所以就分条件
	if(path[7] != 's')
		goto LAZYLOAD;
	kprintf(" [%s]", path);			// 记得删
	/*******************打开文件************************/
	// u32 fd = open(path,"r");	//deleted by mingxuan 2019-5-19
	fd = do_vopen(path, O_RDWR);	//deleted by mingxuan 2019-5-19
	...
	/*************获取elf信息**************/
	Echo_Ehdr = K_PHY2LIN((void *)sys_kmalloc(sizeof(Elf32_Ehdr)));
	read_Ehdr(fd, Echo_Ehdr, 0);
	Echo_Phdr = K_PHY2LIN((void *)sys_kmalloc(sizeof(Elf32_Phdr) * Echo_Ehdr->e_phnum));
...
LAZYLOAD:
	...
	/*************根据elf的program复制文件信息**************/
	if (-1 == exec_lazyload(fd, Echo_Ehdr, Echo_Phdr))
	...
	*((u32*)(p_reg + ESPREG - P_STACKTOP)) = p_proc_current->task.regs.esp;	//added by xw, 17/12/11
	...
	for (addr_lin = p_proc_current->task.memmap.stack_lin_base; addr_lin > p_proc_current->task.memmap.stack_lin_limit; addr_lin -= num_4K)
	{
		PageList page = K_PHY2LIN((void *)sys_kmalloc(sizeof(struct PageNode)));
		page->v_addr = addr_lin & 0xFFFFF000;
		page->flag = PG_UNALLOCATED;
		page->p_addr = 0;
		page->segment = MM_AREA_STACK;
		InsertNode(&p_proc_current->task.pages, &(page->page_node));
		InsertNode(&p_proc_current->task.memmap.stack_pages, &(page->memmap_node));
	}
	//堆    用户还没有申请，所以没有分配，只在PCB表里标示了线性起始位置
...
static u32 exec_load(u32 fd,const Elf32_Ehdr* Echo_Ehdr,const Elf32_Phdr Echo_Phdr[])
{
	...
	for( ph_num=0; ph_num<Echo_Ehdr->e_phnum ; ph_num++ )
	{
		...
		if (Echo_Phdr[ph_num].p_flags == 0x5 || Echo_Phdr[ph_num].p_flags == 0x4)							// xx1，__E, executable seg must be code seg
		{//.text
		...
		else if (Echo_Phdr[ph_num].p_flags == 0x6)						// 1xx，R__, treat all readable but not executable segs as data seg, though some may be RW and some RO
		{//.data
...
/*======================================================================*
*                          exec_lazyload		add by pjt & ljc
*根据elf的program复制文件信息
*======================================================================*/
int exec_lazyload(u32 fd, const Elf32_Ehdr *Echo_Ehdr, const Elf32_Phdr Echo_Phdr[])
{
	u32 ph_num;
	...
	for (ph_num = 0; ph_num < Echo_Ehdr->e_phnum; ph_num++)
	{
		...
		{//.text
			...
			if (map_elf_pglist(fd, Echo_Phdr[ph_num].p_offset, Echo_Phdr[ph_num].p_filesz,
								 Echo_Phdr[ph_num].p_vaddr, Echo_Phdr[ph_num].p_memsz, MM_AREA_TEXT) == -1)
				return -1;
		}
		else if (Echo_Phdr[ph_num].p_flags == 0x6)						// 1xx，R__, treat all readable but not executable segs as data seg, though some may be RW and some RO
		{//.data
			...
			p_proc_current->task.memmap.data_lin_limit = Echo_Phdr[ph_num].p_vaddr + Echo_Phdr[ph_num].p_memsz;
			if (map_elf_pglist(fd, Echo_Phdr[ph_num].p_offset, Echo_Phdr[ph_num].p_filesz,
								 Echo_Phdr[ph_num].p_vaddr, Echo_Phdr[ph_num].p_memsz, MM_AREA_DATA) == -1)
				return -1;
		}
		else 
		{
...
/*======================================================================*
*                          map_elf_pglist		add by pjt & ljc
*给定的文件映射到指定的虚拟地址空间中的页面，
并将相应的页面描述符链接到页面链表和区域的页面链表中
*======================================================================*/
int map_elf_pglist(int fd, u32 offset, u32 filesz, u32 vaddr, u32 memsz, int area)
{
	int i = 0;
	int pgfile_num, pgmem_num;
	PageList pglist;
	LinkList memmap_node;
	pgfile_num = filesz / 4096 + 1;	// 文件所需的页
	pgmem_num = memsz / 4096 + 1;	// 内存所需的页
	pglist = K_PHY2LIN((void *)do_kmalloc(pgmem_num * sizeof(struct PageNode)));
	vaddr = vaddr & 0xFFFFF000;		// 取出它的高20位虚拟地址
	if (MM_AREA_TEXT == area)
	{
		memmap_node = &p_proc_current->task.memmap.text_pages;
	}
	else if (MM_AREA_DATA == area)
	{
		memmap_node = &p_proc_current->task.memmap.data_pages;
	}
	else
	{
		vga_write_str_color(": Type Error\n", 0x74);
		return -1;
	}

	for (; i < pgfile_num - 1; i++, offset += 4096, filesz -= 4096, vaddr += 4096, memsz -= 4096)
	{
		pglist[i].v_addr = vaddr;
		pglist[i].p_addr = 0;
		pglist[i].flag = PG_FILE;
		pglist[i].segment = area;
		pglist[i].fileinfo = (struct FileInfo){fd, offset, 4096};
		write_page_pde(p_proc_current->task.cr3, vaddr, 0x0,
					   PG_USU | PG_RWR);
		InsertNode(&p_proc_current->task.pages, &pglist[i].page_node);
		InsertNode(memmap_node, &pglist[i].memmap_node);
	}
	// 将文件能在4096大小内够刚好映射
	pglist[i].v_addr = vaddr;
	pglist[i].p_addr = 0;
	pglist[i].flag = PG_FILE;
	pglist[i].segment = area;
	pglist[i].fileinfo = (struct FileInfo){fd, offset, filesz};
	write_page_pde(p_proc_current->task.cr3, vaddr, 0x0,
				   PG_USU | PG_RWR);
	InsertNode(&p_proc_current->task.pages, &pglist[i].page_node);
	InsertNode(memmap_node, &pglist[i].memmap_node);
	...
	// 当 pgmem_num > pgfile_num 时
	for (; i < pgmem_num; i++, vaddr += 4096)
	{
		pglist[i].v_addr = vaddr;
		pglist[i].p_addr = 0;
		pglist[i].flag = PG_UNALLOCATED;
		pglist[i].segment = area;
		write_page_pde(p_proc_current->task.cr3, vaddr, 0x0,
					   PG_USU | PG_RWR);
		InsertNode(&p_proc_current->task.pages, &pglist[i].page_node);
