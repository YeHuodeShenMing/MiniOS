...
#define PAGE_FREES_4K	4096

#include "list.h"

struct pgAllocatedInfo
{
	u32 flag;//其实可以用于其他算法使用 如dirty位，但我这里没用到
	u32 pid;
	u32 paddr;
	u32 vaddr;
	u32 segment;
	struct pgAllocatedInfo* pre;
	struct pgAllocatedInfo* next;
};

struct DoubleList
{
	struct pgAllocatedInfo* head;
	struct pgAllocatedInfo* tail;
	u32 size;
};
