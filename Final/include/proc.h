...
typedef struct s_lin_memmap {//线性地址分布结构体	edit by visual 2016.5.25
	...
	struct LinkNode text_pages;				// 代码段
    struct LinkNode data_pages;				// 数据段
	struct LinkNode vpage_pages;			// 保留内存
    struct LinkNode heap_pages;				// 堆
    struct LinkNode stack_pages;			// 栈
	struct LinkNode arg_pages;				// 参数
}LIN_MEMMAP;
