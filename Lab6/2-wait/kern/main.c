...
// pcb表
PROCESS	proc_table[PCB_SIZE];//初始都是IDLE
...
void kernel_main(void)
{
	...
	p_proc->statu = READY;			//程序状态
	// kern regs
	p_proc->kern_regs.esp = (u32)(p_proc_ready + 1) - 8;	//获得内核栈栈顶地址-8
	...
	*(u32 *)(p_proc->kern_regs.esp + 4) = (u32)p_proc;
	...
	switch_kern_context(&idle, &p_proc->kern_regs);	//之后不会跳回来,已经转到新进程了
	assert(0);
}
