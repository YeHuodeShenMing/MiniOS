...
{
	phyaddr_t new_cr3 = phy_malloc_4k();//新的cr3
	memset((void *)K_PHY2LIN(new_cr3), 0, PGSIZE);//对cr3页面进行清0
	struct page_node *new_page_list = kmalloc(sizeof(struct page_node));
	new_page_list->nxt = NULL;
	new_page_list->paddr = new_cr3;//页面列表的第一个结点是cr3页面
	new_page_list->laddr = -1;	//线性地址无法访问
...
kern_exec(PROCESS_0 *p_proc, const char *pathname)
{
	...
	if ((ret = read_file(pathname, (void *)K_PHY2LIN(48 * MB))) < 0)//当读取失败会返回负值
		goto free;
	...
	init_segment_regs(p_proc);//初始化段寄存器
