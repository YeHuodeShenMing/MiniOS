...
loop:
	...
	p_proc->statu = ZOMBIE;		//子进程设置为zombie
	p_fa->statu = READY;
...
{
	PROCESS_0 *p_init = &proc_table[0].pcb;//初始进程
	// 上锁顺序为：初始进程->当前进程->子进程
	while (xchg(&p_init->lock, 1) == 1)//初始进程
		schedule();
	while (xchg(&p_proc->lock, 1) == 1)//当前进程
		schedule();
	//遍历子进程
	for (struct son_node *p = p_proc->fork_tree.sons ; p ;) {
		PROCESS_0 *p_son = p->p_son;//son_node结构体中含有p_son
...
kern_exit(PROCESS_0 *p_proc, int exit_code)
{
	...
	while (xchg(&p_proc->lock, 1) == 1)
		schedule();
	p_proc->exit_code = exit_code;//退出码设置
	xchg(&p_proc->lock, 0);
