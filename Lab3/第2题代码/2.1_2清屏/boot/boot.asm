...
; Hint3: You may want to add about 10 lines of code to more than one place in this file   
   ;mbr中保留的ds：si要在ds赋新值之前取出数据，存在变量中
    mov    bx,word[ds:si+PTItem.startLBA]    
    mov    ax, cs                ; ax <- cs
...
CompareFilename:
    ...
    mov    cx, 10   ; $cx is the length of the string
    push   di 	    ; repe cmpsb affects $di and $si, so push it
    repe   cmpsb    ; compare the 2 strings pointed by $si and $di
    pop    di       ; this does not affect flags, so pop it first to balance the stack
    jcxz   LoaderFound
    dec    dx
    cmp    dx, 0                 
